...
export default function Services() {
  ...
  if (loading) {
    return (
      <div className="min-h-screen bg-gradient-to-br from-slate-950 via-slate-900 to-slate-950 flex items-center justify-center">
        <div className="text-center">
          <div className="w-12 h-12 sm:w-16 sm:h-16 border-4 border-slate-700 border-t-blue-500 rounded-full animate-spin mx-auto" />
          <p className="text-slate-400 text-xs sm:text-sm mt-3 sm:mt-4">Loading services...</p>
        </div>
      </div>
    );
  }
      ...
      <div className="relative max-w-7xl mx-auto px-3 sm:px-4 md:px-6 lg:px-8 py-4 sm:py-6 md:py-8">
        ...
        <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-3 sm:gap-4 mb-4 sm:mb-6 md:mb-8">
          <div className="flex items-center gap-3">
            ...
            <div className="min-w-0">
              ...
              <p className="text-xs sm:text-sm text-slate-400 truncate">Monitor your services in real-time</p>
            </div>
          </div>
          <button
            onClick={loadServices}
            disabled={refreshing}
            aria-label="Refresh services"
            className="flex items-center gap-2 px-3 sm:px-4 py-1.5 sm:py-2 bg-slate-800/50 hover:bg-slate-800 rounded-lg text-xs sm:text-sm text-slate-300 hover:text-white transition-all disabled:opacity-50 flex-shrink-0 self-start sm:self-auto"
          >
        ...
        <div className="grid grid-cols-2 xs:grid-cols-3 sm:grid-cols-5 gap-2 sm:gap-3 md:gap-4 mb-4 sm:mb-6 md:mb-8">
          {[
            ...
            <div key={label} className="bg-slate-900/50 border border-slate-800 rounded-xl p-2 sm:p-3 md:p-4">
        ...
        <div className="flex flex-wrap items-center gap-2 sm:gap-3 md:gap-4 mb-4 sm:mb-6">
          <div className="flex items-center gap-1.5 sm:gap-2">
            ...
            <span className="text-xs sm:text-sm text-slate-400">Filter:</span>
          </div>
          <div className="flex flex-wrap gap-1.5 sm:gap-2">
            {[
              ...
              <button
                key={label}
                onClick={() => navigate(path)}
                className={`px-2.5 sm:px-4 py-1 sm:py-2 rounded-lg text-[10px] sm:text-xs md:text-sm font-medium transition-all focus:outline-none focus:ring-2 focus:ring-blue-500 whitespace-nowrap ${
                  active
                    ? "bg-blue-500/20 text-blue-400 border border-blue-500/30"
                    : "bg-slate-800/50 text-slate-400 hover:text-white hover:bg-slate-800"
                }`}
            ...
            {(statusFilter || anomalyFilter) && (
              <button onClick={() => navigate("/services")} className="text-xs text-slate-400 hover:text-white underline">
                Clear
              </button>
            )}
          </div>
            ...
            {filteredServices.map((service) => {
              ...
              return (
                <button
                  ...
                  onClick={() => navigate(`/services/${service.id}`)}
                  aria-label={`View details for ${service.name}`}
                  className="group w-full text-left bg-slate-900/50 border border-slate-800 rounded-xl p-4 sm:p-5 md:p-6 hover:border-blue-500/50 hover:shadow-lg transition-all duration-300 hover:-translate-y-1 focus:outline-none focus:ring-2 focus:ring-blue-500"
                >
                  <div className="flex items-start justify-between mb-3 sm:mb-4">
                    <div className="flex items-center gap-2 sm:gap-3 min-w-0">
                      ...
                      <div className="min-w-0">
                        ...
                        <p className="text-[10px] sm:text-xs text-slate-500">{typeLabel}</p>
                      </div>
                    </div>
                    <div className="flex items-center gap-1 sm:gap-2 flex-shrink-0">
                      {service.aiStatus?.hasAnomaly && (
                        <div className="flex items-center gap-0.5 sm:gap-1 px-1.5 sm:px-2 py-0.5 sm:py-1 bg-purple-500/20 text-purple-400 border border-purple-500/30 rounded-lg">
                      ...
                      <div
                        ...
                        aria-label={`Delete ${service.name}`}
                        className="p-1.5 sm:p-2 rounded-lg opacity-0 group-hover:opacity-100 hover:bg-red-500/20 text-red-400 transition-all cursor-pointer"
                      >
                  ...
                  <div className="flex items-center gap-2 text-[10px] sm:text-xs text-slate-400 bg-slate-800/30 p-2 sm:p-3 rounded-lg mb-3 sm:mb-4">
                    <LinkIcon className="w-3 h-3 sm:w-3.5 sm:h-3.5 flex-shrink-0" />
                    <span className="truncate">{service.baseUrl}</span>
                  </div>
                  ...
                  <div className="flex items-center justify-between mt-2 sm:mt-3 pt-2 sm:pt-3 border-t border-slate-800 text-[10px] sm:text-xs text-slate-500">
                    <span className="flex items-center gap-1">
        ...
        {serviceToDelete && (
          <div className="fixed inset-0 z-50 flex items-center justify-center p-3 sm:p-4 bg-black/70 backdrop-blur-sm">
            <div className="bg-slate-900 border border-slate-800 rounded-xl w-full max-w-sm sm:max-w-md p-4 sm:p-6 mx-3 sm:mx-4">
              <div className="flex items-center gap-3 mb-3 sm:mb-4">
                <div className="p-2 sm:p-3 bg-red-500/10 rounded-xl border border-red-500/20 flex-shrink-0">
                  <Trash2 className="w-4 h-4 sm:w-5 sm:h-5 text-red-400" />
                </div>
                <div>
                  <h3 className="text-base sm:text-lg font-semibold text-white">Delete Service</h3>
              ...
              <div className="flex flex-col xs:flex-row justify-end gap-2 sm:gap-3">
                <button onClick={() => setServiceToDelete(null)} className="px-3 sm:px-4 py-1.5 sm:py-2 bg-slate-800 hover:bg-slate-700 rounded-lg text-xs sm:text-sm text-white order-2 xs:order-1">
                  Cancel
                </button>
        ...
        {/* Footer */}
        {services.length > 0 && (
          <div className="mt-6 sm:mt-8 pt-3 sm:pt-4 border-t border-slate-800 flex flex-col sm:flex-row items-center justify-between gap-2 sm:gap-4 text-[10px] sm:text-xs text-slate-500">
            <div className="flex items-center gap-2 sm:gap-4 flex-wrap justify-center sm:justify-start">
              <span className="flex items-center gap-1"><span className="w-1.5 h-1.5 sm:w-2 sm:h-2 bg-emerald-400 rounded-full" />{counts.up} Up</span>
              <span className="flex items-center gap-1"><span className="w-1.5 h-1.5 sm:w-2 sm:h-2 bg-yellow-400 rounded-full" />{counts.degraded} Degraded</span>
              <span className="flex items-center gap-1"><span className="w-1.5 h-1.5 sm:w-2 sm:h-2 bg-red-400 rounded-full" />{counts.down} Down</span>
            </div>
            <span className="flex items-center gap-1">
