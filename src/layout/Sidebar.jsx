...
export default function Sidebar() {
  ...
  const navLinks = [
    ...
    { to: "/alerts", icon: Bell, label: "Alerts" },
    { to: "/notifications", icon: Bell, label: "Notifications" },
    { to: "/anomalies", icon: Sparkles, label: "Anomalies" },
  ];
  ...
  if (window.innerWidth < 768) {
    return (
      <>
        ...
        <button
          onClick={() => setIsMobileOpen(!isMobileOpen)}
          className="fixed top-4 left-4 z-50 p-2 bg-white border border-gray-200 rounded-xl text-gray-700 hover:bg-gray-50 hover:border-gray-300 transition-colors md:hidden shadow-sm"
          aria-label="Toggle menu"
        >
          {isMobileOpen ? <X className="w-6 h-6" /> : <Menu className="w-6 h-6" />}
        </button>
      ...
      <>
        ...
        {isDesktop && (
          <button
            onClick={() => setIsCollapsed(!collapsed)}
            aria-label={collapsed ? "Expand sidebar" : "Collapse sidebar"}
            className="absolute -right-3 top-20 z-10 w-6 h-6 bg-white border border-gray-300 rounded-full flex items-center justify-center hover:bg-gray-50 hover:border-gray-400 transition-colors focus:outline-none focus:ring-2 focus:ring-gray-400 shadow-sm"
          >
        ...
        {/* Logo */}
        <div className="relative px-4 sm:px-6 py-4 sm:py-6 border-b border-gray-200">
          <div className="flex items-center gap-3">
            <div className="p-2.5 bg-gradient-to-br from-gray-800 to-gray-900 rounded-xl shadow-lg flex-shrink-0">
              <Activity className="w-5 h-5 text-white" />
            </div>
            <div className={`transition-opacity duration-300 ${collapsed ? "opacity-0 w-0 overflow-hidden" : "opacity-100"}`}>
              <h2 className="text-xl font-bold text-gray-900 whitespace-nowrap">AI Monitor</h2>
              <p className="text-[10px] text-gray-400 tracking-wider uppercase">Enterprise</p>
            </div>
          </div>
        </div>

        {/* Navigation */}
        <nav className="flex-1 px-2 sm:px-3 py-4 sm:py-6 space-y-0.5 overflow-y-auto">
          <div className={`px-3 mb-4 text-xs font-semibold text-gray-400 uppercase tracking-wider transition-opacity ${collapsed ? "opacity-0" : "opacity-100"}`}>
            Main Menu
          </div>
          ...
          {navLinks.map(({ to, icon: Icon, label }) => (
            <NavLink key={to} to={to} className={navItem} onClick={() => setIsMobileOpen(false)}>
              {({ isActive }) => (
                <>
                  <Icon size={20} className={iconClass(isActive)} />
                  <span className={`transition-all duration-300 text-sm font-medium ${collapsed ? "opacity-0 w-0 overflow-hidden" : "opacity-100"}`}>
                    {label}
                  </span>
                  {isActive && !collapsed && (
                    <span className="absolute right-3 w-1 h-6 bg-gray-900 rounded-full shadow-sm" />
                  )}
                </>
              )}
            </NavLink>
        ...
        {/* User Footer */}
        <div className="relative px-4 py-4 sm:py-5 border-t border-gray-200 bg-gray-50/50">
          <div
            className="flex items-center gap-3 cursor-pointer rounded-xl p-2 hover:bg-gray-100 transition-all duration-200"
            onClick={() => setShowMenu(!showMenu)}
          >
            <div className="relative flex-shrink-0">
              <div className="w-10 h-10 rounded-xl bg-gradient-to-br from-gray-800 to-gray-900 flex items-center justify-center text-white font-semibold text-lg shadow-lg">
                {user?.name ? user.name.charAt(0).toUpperCase() : "U"}
              </div>
            ...
            <div className={`transition-all duration-300 ${collapsed ? "opacity-0 w-0 overflow-hidden" : "opacity-100 flex-1 min-w-0"}`}>
              <div className="text-sm font-semibold text-gray-900 truncate max-w-[100px] sm:max-w-[120px]">
                {user?.name || "User"}
              </div>
              <div className="text-xs text-gray-500 truncate max-w-[100px] sm:max-w-[120px]">
                {user?.email || "No email"}
              </div>
              <div className="mt-1.5 flex items-center gap-2">
                <span className="px-2 py-0.5 text-[10px] font-medium bg-gray-100 rounded-full text-gray-600 border border-gray-200 truncate">
                  {user?.role ? user.role.replace("ROLE_", "") : "USER"}
                </span>
              </div>
            </div>
          ...
          {/* Profile Dropdown - responsive */}
          {showMenu && !collapsed && (
            <div className="absolute bottom-24 left-3 right-3 bg-white border border-gray-200 rounded-xl shadow-xl p-4 z-50">
              <div className="flex justify-between items-center mb-3">
                <span className="text-sm font-semibold text-gray-900">Account</span>
                <button
                  onClick={() => setShowMenu(false)}
                  aria-label="Close menu"
                  className="text-gray-400 hover:text-gray-600 text-xs p-2"
                >
                  ✕
                </button>
              </div>
              ...
              <button
                onClick={() => { setShowMenu(false); navigate("/profile"); }}
                className="flex items-center gap-3 w-full text-left text-sm py-2.5 px-3 text-gray-700 hover:text-gray-900 hover:bg-gray-50 rounded-lg transition-colors"
              >
              ...
              <button
                onClick={handleLogout}
                className="flex items-center gap-3 w-full text-left text-sm py-2.5 px-3 text-red-600 hover:text-red-700 hover:bg-red-50 rounded-lg transition-colors mt-1"
              >
